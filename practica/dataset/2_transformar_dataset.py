...
print("# ------------------------------------------------------------------------------")
print("# TRANSFORMAR EL CONJUNTO DE DATOS ORIGINAL")
print("# ------------------------------------------------------------------------------")
...
clasificacion_orden = ['A', 'B', 'C', 'D', 'E', 'F', 'G']
df['clasificacion_emisiones'] = pd.Categorical(df['clasificacion_emisiones'], categories=clasificacion_orden, ordered=True)
df['clasificacion_consumo'] = pd.Categorical(df['clasificacion_consumo'], categories=clasificacion_orden, ordered=True)
...
print("Se eliminan filas con longitud fuera del rango esperado para Aragón...")

print(f"\nDimensiones después realizar las transformaciones: {df.shape[0]} filas x {df.shape[1]} columnas")
...
print("\n# ------------------------------------------------------------------------------")
print("# GENERAR UN NUEVO DATAFRAME AGREGADO POR MUNICIPIO")
print("# ------------------------------------------------------------------------------")
...
# Calcular nuevas columnas 'clasificacion_emisiones_FG_pct', 'clasificacion_consumo_FG_pct', 'clasificacion_emisiones_EFG_pct', 'clasificacion_consumo_EFG_pct'
df_final['clasificacion_emisiones_FG_pct'] = df_final['clasificacion_emisiones_F_pct'] + df_final['clasificacion_emisiones_G_pct']
df_final['clasificacion_consumo_FG_pct'] = df_final['clasificacion_consumo_F_pct'] + df_final['clasificacion_consumo_G_pct']
df_final['clasificacion_emisiones_EFG_pct'] = df_final['clasificacion_emisiones_E_pct'] + df_final['clasificacion_emisiones_FG_pct']
df_final['clasificacion_consumo_EFG_pct'] = df_final['clasificacion_consumo_E_pct'] + df_final['clasificacion_consumo_FG_pct']

print(df_final.info())

print(df_final[['clasificacion_emisiones_FG_pct', 'clasificacion_consumo_FG_pct', 'clasificacion_emisiones_EFG_pct', 'clasificacion_consumo_EFG_pct']].describe())

# Guardar el dataframe final en un nuevo archivo CSV
if GUARDAR_CSV_AGREGADO:
...
if GRAFICOS:
    ...
    colores_clasificacion = {
        'A': '#008000',  # verde oscuro
        'B': '#4CAF50',  # verde
        'C': '#8BC34A',  # verde claro
        'D': '#CDDC39',  # amarillo verdoso
        'E': '#FFEB3B',  # amarillo
        'F': '#FF9800',  # naranja
        'G': '#F44336'   # rojo
    }
    ...
    # --------------------------------------------------------------------------
    # Diagrama de barras por clasificacion_consumo
    # --------------------------------------------------------------------------
    plt.figure(figsize=(10, 7))
    sns.countplot(
        data=df,
        x='clasificacion_consumo',
        order=clasificacion_orden,
        palette=[colores_clasificacion[c] for c in clasificacion_orden]
    )
    plt.title('Número de certificados por clasificación de consumo energético')
    plt.xlabel('Clasificación de consumo energético')
    plt.ylabel('Número de certificados')
    plt.tight_layout()
    plt.savefig('img/countplot_consumo.png', dpi=300)
    plt.show()

    # --------------------------------------------------------------------------
    # Gráfico circular con el porcentaje de cada clasificacion_consumo
    # --------------------------------------------------------------------------
    consumo_counts = df['clasificacion_consumo'].value_counts().reindex(clasificacion_orden, fill_value=0)
    plt.figure(figsize=(8, 8))
    plt.pie(consumo_counts, labels=consumo_counts.index, autopct='%1.1f%%', startangle=140, colors=[colores_clasificacion[c] for c in consumo_counts.index])
    plt.title('Porcentaje de certificados por clasificación de consumo energético')
    plt.tight_layout()
    plt.savefig('img/pie_consumo.png', dpi=300)
    plt.show()
    ...
    plt.title('Año de construcción según clasificación energética')
    plt.xlabel('Clasificación de consumo energético')
    plt.ylabel('Año de construcción')
    plt.ylim(bottom=1900)
    plt.ylim(top=2030)

    plt.tight_layout()
    plt.savefig('img/boxplot_anio_consumo.png', dpi=300)
    plt.show()

    # --------------------------------------------------------------------------
    # Gráfico de barras apiladas por provincia y clasificacion_consumo
    # --------------------------------------------------------------------------
    tabla_pct = pd.crosstab(
        df['prov'],
        df['clasificacion_consumo'],
        normalize='index'
    ) * 100

    tabla_pct = tabla_pct[clasificacion_orden]

    ax = tabla_pct.plot(
        kind='bar',
        stacked=True,
        figsize=(10, 7),
        color=[colores_clasificacion[c] for c in clasificacion_orden]
    )

    # Añadir porcentaje dentro de cada segmento
    for container in ax.containers:
        labels = [
            f"{bar.get_height():.1f}%"
            if bar.get_height() >= 3 else ""  # ocultar segmentos pequeños
    ...
    plt.title('Distribución porcentual de clasificaciones energéticas por provincia')
    plt.xlabel('Provincia')
    plt.ylabel('Porcentaje (%)')
    plt.legend(title='Clasificación')

    plt.tight_layout()
    plt.savefig('img/bar_consumo_provincia_pct.png', dpi=300)
    plt.show()